...
- [ ] Tearsheet gaps — `q.plot.report()` covers the standard quantstats layout (cumulative returns linear/log/vol-matched, EOY bars, monthly distribution, daily returns, rolling beta/vol/Sharpe/Sortino, worst drawdown periods, underwater, monthly heatmap, metrics + EOY + worst-drawdowns tables); still missing vs. quantstats' full report:
  - [ ] Extended metrics table rows — "full" mode: Prob./Smart Sharpe & Sortino, Omega, VaR/CVaR, Kelly criterion, payoff ratio, profit factor, tail/common-sense ratios, expected daily/monthly/yearly return, MTD/3M/6M/YTD/1Y period returns, consecutive win/loss streaks, exposure (quantstats `reports.metrics(mode="full")`)
